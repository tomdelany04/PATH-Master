...
trial_summary %>%
gt() %>%
tab_caption("Overview of GUSTO and DIG trials") %>%
...
We plan to conduct further analysis on the International Stroke Trial (IST) [^5] and the National Lung screening Trial (NLST) [^6], to evaluate the generalizability of PATH across different clinical domains (Table 2). We also plan to implement a machine-learning approach using causal forests for patient risk prediction. We will use the `grf` package for this.
...
trial_summary %>%
gt() %>%
tab_caption("Overview of IST and NLST trials") %>%
