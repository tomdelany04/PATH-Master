...
```{r descriptivetable, out.width='80%'}
table1(~DEATH + AGE + Ejection_Fraction + Classification, data =dig.df,  digits =2)
```
...
res2 <- readRDS("dig_forest_baseline_risk_model.rds")
print(res2)
...
```{r fig-proportional, echo=FALSE, message = FALSE, warning= FALSE, fig.cap="DIG-Trial, Absolute treatment benefit of Digoxin increases with baseline risk", out.width="100%", fig.align="center"}

saveRDS(res, file = "dig_HTE.rds")
res2 <- readRDS("dig_HTE.rds")
print(res2)

pack3 <- readRDS("dig_HTE.rds")
...
plot(x = xp, y = p1exp, type = 'l', lty = 2, lwd = 3, 
     xlim = c(0.1, 0.7), ylim = c(-0.06, 0.06), col = "#84003d",
     xlab = "Baseline Risk (Placebo-based)", 
     ylab = "Absolute Benefit by Digoxin(Lives Saved)", 
     cex.lab = 1.2, las = 1, bty = 'l')

# Add horizontal reference line (Zero absolute benefit)
abline(h = 0, col = "#84003d", lwd = 1) 

# The "Lake Wobegon" effect visualisation: linear predictor calculated as if every patient were untreated
histSpike(plogis(lp.no.tx), add = TRUE, side = 1, nint = 100, frac = 0.15) 

# 6. Plot observed data points from risk quarters
# rate0 = mortality in placebo quarters; ratediff = Absolute Risk Difference (ARD)
points(x = rate0, y = ratediff, pch = 1, cex = 1.8, lwd = 2, col = "blue")
arrows(x0 = rate0, x1 = rate0, y0 = CI[,2], y1 = CI[,3], 
       angle = 90, code = 3, len = 0.1, col = "blue")

# Add Legend
legend("topleft", lty = c(2, NA), pch = c(NA, 1), lwd = c(3, 2), 
       bty = 'n', col = c("#84003d", "blue"), cex = 1.1,
       legend = c("Expected with proportional effect", 
                  "Observed (Risk quarters)"))
