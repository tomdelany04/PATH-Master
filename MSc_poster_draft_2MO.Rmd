...
```{r fig-proportional-dig, echo=FALSE, message = FALSE, warning= FALSE, fig.cap="DIG-Trial, Absolute treatment benefit of Digoxin increases with baseline risk", out.width="100%", fig.align="center"}
...
# proportional-effect coefficient for Digoxin
btx <- unname(coef(model1_dig)["Digoxin"])

# Expected benefit curve: p0 - p1 where logit(p1) = logit(p0) + btx
p1exp <- plogis(logxp0) - plogis(logxp0 + btx)
...
lines(x = c(0, .35), y = c(0, 0), col = "#84003d")

# distribution of placebo-based baseline risks
histSpike(plogis(dig_clean$lp), add = TRUE, side = 1, nint = 300, frac = .15)

# x-axis points are placebo risks by quartile (rate0)
points(x = as.numeric(rate0), y = ratediff, pch = 1, cex = 1.5, lwd = 2, col = "blue")
arrows(x0 = as.numeric(rate0), x1 = as.numeric(rate0),
       y0 = CI_quarters[, 2], y1 = CI_quarters[, 3],
       angle = 90, code = 3, len = .1, col = "blue")
